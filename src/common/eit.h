...
#if defined(__cplusplus)
#define SDB_BEGIN_EXTERN_C extern "C" {
...
static inline const char *SdbStrErr(sdb_errno E)
{
	if (E < 0) {
		E = -E;
	}
...
static_assert(SDB_LOG_BUF_SIZE >= 128,
	      "SDB_LOG_BUF_SIZE must greater than or equal to 128!");
...
i64 Sdb__WriteLog__(sdb__log_module__ *Module, const char *LogLevel,
		    const char *Fmt, ...) __attribute__((format(printf, 3, 4)));

#define SDB__LOG_LEVEL_CHECK__(level) (SDB_LOG_LEVEL >= SDB_LOG_LEVEL_##level)

#define SDB_LOGGING_NOT_USED                         \
	static sdb__log_module__ *Sdb__LogInstance__ \
		__attribute__((used)) = NULL

#define SDB_LOG_REGISTER(module_name)                                         \
	static char SDB_CONCAT3(Sdb__LogModule, module_name,                  \
				Buffer__)[SDB_LOG_BUF_SIZE];                  \
	sdb__log_module__ SDB_CONCAT3(Sdb__LogModule, module_name,            \
...
#define SDB_LOG_DECLARE(name)                                           \
	...
	static sdb__log_module__ *Sdb__LogInstance__                    \
		__attribute__((used)) = &SDB_CONCAT3(Sdb__LogModule, name, __)

#define SDB__LOG__(log_level, fmt, ...)                                       \
	do {                                                                  \
		if (SDB__LOG_LEVEL_CHECK__(log_level)) {                      \
			sdb_errno LogRet = Sdb__WriteLog__(                   \
				Sdb__LogInstance__, SDB_STRINGIFY(log_level), \
				fmt, ##__VA_ARGS__);                          \
			assert(LogRet >= 0);                                  \
		}                                                             \
	} while (0)
...
#define SdbAssert(condition, fmt, ...)                                    \
	do {                                                              \
		if (!(condition)) {                                       \
			Sdb__WriteLog__(Sdb__LogInstance__, "DBG",        \
...
#define SDB_THREAD_ARENAS_REGISTER(thread_name, count)                         \
	static __thread sdb_arena *SDB_CONCAT3(Sdb__ThreadArenas, thread_name, \
					       Buffer__)[count]                \
		__attribute__((used));                                         \
	__thread sdb__thread_arenas__ SDB_CONCAT3(Sdb__ThreadArenas,           \
						  thread_name, __)             \
...
#define SDB_THREAD_ARENAS_EXTERN(thread_name)                               \
	extern __thread sdb__thread_arenas__ SDB_CONCAT3(Sdb__ThreadArenas, \
							 thread_name, __);  \
	static __thread sdb__thread_arenas__ *Sdb__ThreadArenasInstance__   \
		__attribute__((used))
...
#define SdbThreadArenasInit(thread_name)                               \
	Sdb__ThreadArenasInit__(                                       \
		SDB_CONCAT3(Sdb__ThreadArenas, thread_name, Buffer__), \
		&SDB_CONCAT3(Sdb__ThreadArenas, thread_name, __),      \
		&Sdb__ThreadArenasInstance__)

void Sdb__ThreadArenasInitExtern__(sdb__thread_arenas__ *TAs,
				   sdb__thread_arenas__ **TAInstance);
#define SdbThreadArenasInitExtern(thread_name)                        \
	Sdb__ThreadArenasInitExtern__(&SDB_CONCAT3(Sdb__ThreadArenas, \
						   thread_name, __),  \
				      &Sdb__ThreadArenasInstance__)

sdb_errno Sdb__ThreadArenasAdd__(sdb_arena *Arena,
				 sdb__thread_arenas__ *TAInstance);
#define SdbThreadArenasAdd(arena) \
...
#define SdbScratchGet(conflicts, conflict_count)     \
	Sdb__ScratchGet__(conflicts, conflict_count, \
			  Sdb__ThreadArenasInstance__)
...
#define SdbPushArray(arena, type, count) \
	(type *)SdbArenaPush(arena, sizeof(type) * (count))
#define SdbPushArrayZero(arena, type, count) \
...
#define SdbArrayDeleteAndShift(mem, i, count, esize) \
...
bool SdbWriteBufferToFile(void *Buffer, u64 ElementSize, u64 ElementCount,
			  const char *Filename);
bool SdbWrite_sdb_file_data_ToFile(sdb_file_data *FileData,
				   const char *Filename);
 ...
 */
sdb_errno Sdb__MemMap__(sdb_mmap *Map, void *MAddr, size_t MSize, int MProt,
			int MFlags, int MFd, off_t MOffset, sdb_string FileName,
			int OFlags, mode_t OMode, sdb__log_module__ *Module);

#define SdbMemMap(map, maddr, msize, mprot, mflags, mfd, moffset, filename, \
		  oflags, omode)                                            \
	Sdb__MemMap__(map, maddr, msize, mprot, mflags, mfd, moffset,       \
		      filename, oflags, omode, Sdb__LogInstance__)
...
void *Sdb__MallocTrace__(size_t Size, int Line, const char *Func,
			 sdb__log_module__ *Module);
void *Sdb__CallocTrace__(u64 ElementCount, u64 ElementSize, int Line,
			 const char *Func, sdb__log_module__ *Module);
void *Sdb__ReallocTrace__(void *Pointer, size_t Size, int Line,
			  const char *Func, sdb__log_module__ *Module);
void Sdb__FreeTrace__(void *Pointer, int Line, const char *Func,
		      sdb__log_module__ *Module);

#if SDB_MEM_TRACE == 1
#define malloc(Size) \
	Sdb__MallocTrace__(Size, __LINE__, __func__, Sdb__LogInstance__)
#define calloc(Count, Size) \
	Sdb__CallocTrace__(Count, Size, __LINE__, __func__, Sdb__LogInstance__)
#define realloc(Pointer, Size)                                 \
	Sdb__ReallocTrace__(Pointer, Size, __LINE__, __func__, \
			    Sdb__LogInstance__)
#define free(Pointer) \
	Sdb__FreeTrace__(Pointer, __LINE__, __func__, Sdb__LogInstance__)
#endif
		    ...
		    const char *Fmt, ...)
{
	...
	if (FormatRet < 0) {
		...
		(void)SdbMemset(Module->Buffer + CharsWritten, 0,
				BufferRemaining);
...
void SdbMemZero(void *__restrict Mem, u64 Size)
{
...
void Sdb__StringSetLen__(sdb_string String, u64 Len)
{
	SDB_STRING_HEADER(String)->Len = Len;
}

void Sdb__StringSetCap__(sdb_string String, u64 Cap)
{
	SDB_STRING_HEADER(String)->Cap = Cap;
}

sdb_string Sdb__StringMake__(sdb_arena *A, const void *InitString, u64 Len)
{
...
sdb_string Sdb__StringAppend__(sdb_string String, const void *Other,
			       u64 OtherLen)
{
...
sdb_string SdbStringAppendFmt(sdb_string String, const char *Fmt, ...)
{
	...
	va_list Va;
	va_start(Va, Fmt);
	u64 FmtLen = vsnprintf(Buf, SdbArrayLen(Buf) - 1, Fmt,
			...
			int OFlags, mode_t OMode, sdb__log_module__ *Module)
{
	...
	if (Map->Data == MAP_FAILED) {
		Sdb__WriteLog__(Module, "ERR", "Error mapping memory: %s",
				strerror(errno));
		goto error;
	}
			 ...
			 sdb__log_module__ *Module)
{
	...
	Sdb__WriteLog__(Module, "DBG", "MALLOC (%s,%d): %p (%zd B)", Func, Line,
			Pointer, Size);
	return Pointer;
}
			  ...
			  const char *Func, sdb__log_module__ *Module)
{
	...
	Sdb__WriteLog__(Module, "DBG", "REALLOC (%s,%d): %p -> %p (%zd B)",
