...
GenerateShaftPowerData(shaft_power_data *Data)
{
    ...
    Data->PacketId    = Id++;
    Data->Time        = time(NULL);
    Data->Rpm         = (pg_float8)Rpm++;
    Data->Torque      = (pg_float8)Torque++;
    Data->Power       = (pg_float8)Power++;
    Data->PeakPeakPfs = (pg_float8)PeakPeakPfs++;
}
...
GenerateShaftPowerDataRandom(shaft_power_data *Data)
{
    ...
    Data->Rpm         = ((rand() % 100) + 30.0) * sin((double)Id * 0.1); // RPM with variation
    Data->Torque      = ((rand() % 600) + 100.0) * 0.8;                  // Torque (Nm)
    Data->Power       = Data->Rpm * Data->Torque / 9.5488;               // Derived power (kW)
    Data->PeakPeakPfs = ((rand() % 50) + 25.0) * cos((double)Id * 0.1);  // PFS variation
}
...
{
    Buffer[0] = (u8)(TransactionId >> 8) & 0xFF;
    Buffer[1] = (u8)TransactionId & 0xFF;
    Buffer[2] = (u8)(ProtocolId >> 8) & 0xFF;
    Buffer[3] = (u8)ProtocolId & 0xFF;
    Buffer[4] = (u8)(Length >> 8) & 0xFF;
    Buffer[5] = (u8)Length & 0xFF;
    Buffer[6] = UnitId;
    Buffer[7] = (u8)FunctionCode;
...
SendModbusData(int NewFd)
{
    ...
    ModbusFrame[Pos++] = (u8)(Length >> 8) & 0xFF; // Length high
    ModbusFrame[Pos++] = (u8)Length & 0xFF;        // Length low
    ModbusFrame[Pos++] = 0x01;                     // Unit ID
    ModbusFrame[Pos++] = 0x10;                     // Function code
    ModbusFrame[Pos++] = (u8)DataLength;           // Byte count
