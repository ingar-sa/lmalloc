...
sdb_errno PgRun(void *Arg) {
  ...
  SdhsArena *PgArena =
      ArenaCreate(PgASize, SDHS_ARENA_TEST_IS_CONTIGUOUS,
  ...
  for (u64 s = 0; s < PG_SCRATCH_COUNT; ++s) {
    SdhsArena *Scratch =
        ArenaBootstrap(PgArena, NULL, Ctx->PgScratchSize, PgArena->alignment);
  ...
  struct epoll_event ReadEvent = {.events = EPOLLIN | EPOLLERR | EPOLLHUP,
                                  .data.fd = ReadEventFd};
  ...
  while (!SdbShouldShutdown()) {
    ...
    if (EpollRet == -1) {
      ...
      SdbLogInfo("Epoll wait timed out for the %lusthnd time",
                 ++TimeoutCounter);
      if (TimeoutCounter >= 5) {
        SdbLogError(
            "Epoll has timed out more than threshold. Stopping main loop");
        Ret = -ETIMEDOUT;
      } else {
        continue;
      }
    ...
    if (Events[0].events & EPOLLIN) {
      while ((Buf = SdPipeGetReadBuffer(Pipe)) != NULL) {
        ...
        sdb_errno InsertRet = PgInsertData(
            Conn, TableInfo, (const char *)ArenaBase(Buf), ItemCount);
        ...
        if (InsertRet != 0) {
          SdbLogError("Failed to insert data for the %lusthnd",
                      ++PgFailCounter);
          if (PgFailCounter >= 5) {
            SdbLogError(
                "Postgres operations have failed more than threshod. Stopping "
                "main loop");
            Ret = -1;
            break;
          }
