...
sdb_errno Sdb__WriteLog__(sdb__log_module__ *Module, const char *LogLevel, const char *Fmt, ...)
...
char *SdbStrdup(char *String, SdhsArena *Arena);
...
sdb_string SdbStringMake(SdhsArena *A, const char *String);
void       SdbStringFree(sdb_string String);

sdb_string SdbStringDuplicate(SdhsArena *A, const sdb_string String);
...
sdb_file_data *SdbLoadFileIntoMemory(const char *Filename, SdhsArena *Arena);
...
#ifndef SDB_MEM_TRACE
#define SDB_MEM_TRACE 0
#endif
...
Sdb__WriteLog__(sdb__log_module__ *Module, const char *LogLevel, const char *Fmt, ...)
{
    ...
    ssize_t FormatRet = (ssize_t)strftime(Module->Buffer, BufferRemaining, "%T: ", &TimeInfo);
    if(0 == FormatRet) {
    ...
    BufferRemaining -= (size_t)FormatRet;
    ...
    if((ssize_t)(-1) == write(OutFd, Module->Buffer, (size_t)CharsWritten)) {
        return -errno;
    }
...
SdbMemcmp(const void *Lhs, const void *Rhs, size_t Len)
{
    ...
    for(size_t i = 0; i < Len; ++i) {
        if(((const u8 *)Lhs)[i] != ((const u8 *)Rhs)[i]) {
            return false;
        }
    }
    return true;
...
char *
SdbStrdup(char *String, SdhsArena *Arena)
{
    u64   StringLength = SdbStrlen(String);
    char *NewString    = ArenaPushArray(Arena, char, StringLength + 1);
...
#define SDB_STRING_HEADER(str) ((sdb_string_header *)((uintptr_t)(str) - sizeof(sdb_string_header)))
...
static sdb_string
Sdb__StringMake__(SdhsArena *A, const void *InitString, u64 Len)
{
    size_t HeaderSize = sizeof(sdb_string_header);
    void  *Ptr        = ArenaAlloc(A, HeaderSize + Len + 1);
    if(Ptr == NULL) {
        return NULL;
    }
    if(InitString == NULL) {
...
sdb_string
SdbStringMake(SdhsArena *A, const char *String)
{
...
SdbStringFree(sdb_string String)
{
    if(String != NULL) {
        ...
        ArenaPop(H->Arena, PopSize);
    }
}

sdb_string
SdbStringDuplicate(SdhsArena *A, const sdb_string String)
{
...
SdbStringMakeSpace(sdb_string String, u64 AddLen)
{
    ...
    if(Available < AddLen) {
        SdhsArena *A        = SDB_STRING_HEADER(String)->Arena;
        u64        Reserved = ArenaReserve(A, AddLen);
        if(Reserved < AddLen) {
            return NULL;
        }
...
SdbStringAppendFmt(sdb_string String, const char *Fmt, ...)
{
    ...
    va_list Va;
    va_start(Va, Fmt);
    u64 FmtLen = (u64)vsnprintf(Buf, SdbArrayLen(Buf) - 1, Fmt, Va); // TODO(ingar): stb_sprintf?
    va_end(Va);
              ...
              sdb__log_module__ *Module)
{
    ...
    if(OFlags & O_TRUNC || (MFlags & MAP_SHARED && MProt & PROT_WRITE)) {
        if(ftruncate(Map->Fd, (long)MSize) == -1) {
...
sdb_file_data *
SdbLoadFileIntoMemory(const char *Filename, SdhsArena *Arena)
{
    ...
    if(NULL != Arena) {
        FileData = ArenaAlloc(Arena, FileDataSize);
    } else {
