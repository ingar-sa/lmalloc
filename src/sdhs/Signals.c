...
static pid_t GetThreadId(void) { return (pid_t)syscall(SYS_gettid); }
 ...
 */
static void DumpProcessMemory(int SignalNum, siginfo_t *Info) {
  ...
  snprintf(DumpFilename, sizeof(DumpFilename),
           ...
           TmInfo->tm_hour, TmInfo->tm_min, TmInfo->tm_sec, SignalNum,
           ThreadId);
  ...
  snprintf(Header, sizeof(Header),
           ...
           "Fault Address: %p\n"
           "Process ID: %d\n\n",
           ctime(&Now), SignalNum, ThreadId, ThreadName, Info->si_addr,
           getpid());
 ...
 */
static void InitiateGracefulShutdown(void) {
 ...
 */
static bool HandlePipeDump(void) {
  ...
  snprintf(DumpFilename, sizeof(DumpFilename),
           "dumps/pipe_dump_%04d%02d%02d_%02d%02d%02d.bin",
           TmInfo->tm_year + 1900, TmInfo->tm_mon + 1, TmInfo->tm_mday,
 ...
 */
static void SignalHandler(int SignalNum, siginfo_t *Info, void *Context) {
  switch (SignalNum) {
  case SIGINT:
  case SIGTERM: {
...
int SdbSetupSignalHandlers(struct tg_manager *Manager) {
  ...
  for (size_t i = 0; i < sizeof(SignalsToHandle) / sizeof(SignalsToHandle[0]);
       i++) {
    if (sigaction(SignalsToHandle[i], &SignalAction, NULL) == -1) {
      SdbLogError("Failed to set up signal handler for %d: %s",
                  SignalsToHandle[i], strerror(errno));
      return -1;
    }
  }
...
static void SdbSetMemoryDumpPath(const char *Path) {
...
static bool SdbIsShutdownInProgress(void) {
...
bool SdbDumpSensorDataPipe(sensor_data_pipe *Pipe, const char *Filename) {
  ...
  //   SdhsArena *Buffer = Pipe->Buffers[i];
  //   // fwrite(&ArenaPos(Buffer), sizeof(ArenaPos(Buffer)), 1, DumpFile);
  //   fwrite(&Buffer->cur, sizeof(Buffer->cur), 1, DumpFile);
  //
  //   if (Buffer->cur > 0) {
  //     // if (ArenaPos(Buffer) > 0) {
  //     fwrite(Buffer->mem, 1, Buffer->cur, DumpFile);
  //     // fwrite(Buffer->mem, 1, ArenaPos(Buffer), DumpFile);
  //   }
  // }
...
void ConvertDumpToCSV(const char *DumpFileName, const char *CsvFileName) {
  ...
  while (fread(&Packet, sizeof(shaft_power_data), 1, DumpFile) == 1) {
    fprintf(CsvFile, "%ld,%.2ld,%.2lf,%.2lf,%.2lf,%.2lf\n", Packet.PacketId,
            Packet.Time, Packet.Rpm, Packet.Torque, Packet.Power,
            Packet.PeakPeakPfs);
  }
