...
void PgInitThreadArenas(void) { ThreadArenasInit(Postgres); }
...
postgres_ctx *PgPrepareCtx(SdhsArena *PgArena, sensor_data_pipe *Pipe) {
  ...
  cJSON *SchemaConf =
      DbInitGetConfFromFile("./configs/sensor_schemas.json", Scratch.ua);
  ...
  cJSON_ArrayForEach(SensorSchema, SensorSchemaArray) {
    ...
    sdb_string CreationQuery =
        SdbStringMake(Scratch.ua, "CREATE TABLE IF NOT EXISTS ");
    ...
    Ti->ColMetadata =
        GetTableMetadata(PgCtx->DbConn, Ti->TableName, &Ti->ColCount,
    ...
    // Pipe->ItemMaxCount = ArenaCap(Pipe->Buffers[0]) / Pipe->PacketSize;
    Pipe->ItemMaxCount = ArenaCap(Pipe->Buffers[0])/ Pipe->PacketSize;
...
sdb_errno PgInsertData(PGconn *Conn, pg_table_info *Ti, const char *Data,
  ...
  if (PQresultStatus(PgRes) != PGRES_COPY_IN) {
    SdbLogError("Failed to start COPY operation for table %s. Pg error: %s",
                Ti->TableName, PQerrorMessage(Conn));
    PQclear(PgRes);
  ...
  if (PQputCopyData(Conn, CopyHeader, 19) != 1) {
    SdbLogError("Failed to send COPY binary header for table %s. Pg error: %s",
                Ti->TableName, PQerrorMessage(Conn));
    Ret = -SDBE_PG_ERR;
    goto cleanup;
  }

  size_t PgHeaderSize = sizeof(Ti->ColCount) +
  ...
  if (NtwrkConvBuf == NULL) {
    SdbLogError("Scratch arena has insufficient space for network conversion "
                "buffer (need "
  ...
  for (u64 RowsProcessed = 0; RowsProcessed < ItemCount; ++RowsProcessed) {
    ...
    i16 NtwrkFieldCount = (i16)htons((u16)Ti->ColCountNoAutoIncrements);
    ...
    for (int c = 0; c < Ti->ColCount; ++c) {
      ...
      case PG_TIMESTAMP:
        ...
        break;
      default:
        SdbLogWarning("Encountered unhandled or invalid PG oid %d",
                      ColMd.TypeOid);
        break;
      }
    }
  ...
  if (PQputCopyData(Conn, NtwrkConvBuf, (int)ConvOffset) != 1) {
    SdbLogError("Unable to copy converted data for table %s. Pg error: %s",
                Ti->TableName, PQerrorMessage(Conn));
...
cleanup:
  ...
  if (PQresultStatus(PgRes) != PGRES_COMMAND_OK) {
    SdbLogError("Failed to end transaction for table %s. Pg error: %s",
                Ti->TableName, PQerrorMessage(Conn));
    Ret = -SDBE_PG_ERR;
  }
  PQclear(PgRes);
