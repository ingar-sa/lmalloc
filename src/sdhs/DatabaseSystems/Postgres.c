...
THREAD_ARENAS_REGISTER(Postgres, 2);
 ...
 */
inline pg_timestamp UnixToPgTimestamp(time_t UnixTime)
{
	pg_timestamp Timestamp =
		((UnixTime * USECS_PER_SECOND) +
...
void DiagnoseConnectionAndTable(PGconn *DbConn, const char *TableName)
{
	...
	PGresult *TableCheckResult = PQexecParams(
		DbConn, TableCheckQuery, 1, NULL, ParamValues, NULL, NULL, 0);

	if (PQresultStatus(TableCheckResult) == PGRES_TUPLES_OK &&
	    PQntuples(TableCheckResult) > 0) {
		SdbLogInfo("Table '%s' exists: %s", TableName,
			   strcmp(PQgetvalue(TableCheckResult, 0, 0), "t") ==
					   0 ?
				   "Yes" :
				   "No");
	} else {
		SdbLogError("Error checking table existence: %s",
			    PQerrorMessage(DbConn));
	}
	PQclear(TableCheckResult);

	// List all tables in public schema
	const char *ListTablesQuery =
...
{
	// NOTE(ingar): Since it's a printing function, using printf instead of
	// logging functions is
...
pg_col_metadata *GetTableMetadata(PGconn *DbConn, sdb_string TableName,
				  i16 *ColCount, i16 *ColCountNoAutoIncrements,
				  size_t *RowSize, SdhsArena *A)
{
	SdhsArena *Conflicts[1] = { A };
	SdhsArenaScratch Scratch = ScratchGet(Conflicts, 1);

	sdb_string MetadataQuery = SdbStringMake(Scratch.ua, NULL);
	SdbStringAppendFmt(MetadataQuery, PQ_TABLE_METADATA_QUERY_FMT,
			   TableName);

	PGresult *Result = PQexec(DbConn, MetadataQuery);
	if (PQresultStatus(Result) != PGRES_TUPLES_OK) {
		SdbLogError("Metadata query failed: %s",
			    PQerrorMessage(DbConn));
		PQclear(Result);
	...
	// NOTE(ingar): Each row in the result is the metadata for one column in the
	// requested table
	*ColCount = (i16)RowCount;

	pg_col_metadata *MetadataArray =
		ArenaPushArray(A, pg_col_metadata, (size_t)RowCount);
	if (!MetadataArray) {
	...
	*ColCountNoAutoIncrements = 0;
	...
	for (int RowIndex = 0; RowIndex < RowCount; ++RowIndex) {
		...
		CurrentMetadata->ColumnName =
			SdbStringMake(A, PQgetvalue(Result, RowIndex, 0));
		CurrentMetadata->TypeOid =
			(pg_oid)atoi(PQgetvalue(Result, RowIndex, 1));
		CurrentMetadata->TypeLength =
			(i32)atoi(PQgetvalue(Result, RowIndex, 2));
		CurrentMetadata->TypeModifier =
			(i32)atoi(PQgetvalue(Result, RowIndex, 3));
		CurrentMetadata->IsPrimaryKey =
			(PQgetvalue(Result, RowIndex, 4)[0] == 't');
		CurrentMetadata->IsAutoIncrement =
			(PQgetvalue(Result, RowIndex, 5)[0] == 't');

		if (CurrentMetadata->IsAutoIncrement) {
			// NOTE(ingar): The assumption is that auto-incrementing values will not
			// appear in the incoming data
		...
		*RowSize += (size_t)CurrentMetadata->TypeLength;
...
postgres_ctx *PgPrepareCtx(SdhsArena *PgArena, sensor_data_pipe *Pipe)
{
	...
	sdb_file_data *ConfFile =
		SdbLoadFileIntoMemory(POSTGRES_CONF_FS_PATH, Scratch.ua);
	if (ConfFile == NULL) {
	...
	cJSON *SchemaConf = DbInitGetConfFromFile(
		"./configs/sensor_schemas.json", Scratch.ua);
	...
	PgCtx->TablesInfo =
		ArenaPushArray(PgArena, pg_table_info *, SensorCount);
	...
	if (PQstatus(PgCtx->DbConn) != CONNECTION_OK) {
		SdbLogError(
			"PgCtx->DbConnection to database failed. PQ error:\n%s",
	...
	cJSON_ArrayForEach(SensorSchema, SensorSchemaArray)
	{
		...
		pg_table_info *Ti = ArenaPushStruct(PgArena, pg_table_info);
		Ti->TableName = SdbStringMake(PgArena, SensorName->valuestring);
		PgCtx->TablesInfo[SensorIdx] = Ti;

		sdb_string CreationQuery = SdbStringMake(
			Scratch.ua, "CREATE TABLE IF NOT EXISTS ");
		...
		if (PQresultStatus(PgRes) != PGRES_COMMAND_OK) {
			SdbLogError("Table creation failed: %s",
				    PQerrorMessage(PgCtx->DbConn));
			Errno = -SDBE_PG_ERR;
			PQclear(PgRes);
			goto cleanup;
			...
			SdbLogInfo(
				"Table '%s' created successfully (or it already existed).",
				SensorName->valuestring);
			PQclear(PgRes);
		}

		Ti->ColMetadata = GetTableMetadata(
			PgCtx->DbConn, Ti->TableName, &Ti->ColCount,
			&Ti->ColCountNoAutoIncrements, &Ti->RowSize, PgArena);

		if (NULL == Ti->ColMetadata || 0 == Ti->ColCount) {
			SdbLogError(
				"Failed to get column metadata for table %s",
				Ti->TableName);
			Errno = -SDBE_PG_ERR;
			goto cleanup;
		}
		...
		for (int c = 0; c < Ti->ColCount; ++c) {
			...
			if (!ColMd.IsAutoIncrement) {
				SdbStringAppend(Ti->CopyCommand,
						ColMd.ColumnName);
				SdbStringAppendC(Ti->CopyCommand, ", ");
			}
		}
		...
		// NOTE(ingar): An assumption made is that each sensor will have its own
		// pipe since we don't have a method of differentiating packets at the
		// moment, but unfortunately we probably don't have time to complete this
		// part. This means that the max number of sensors supported in this current
		// implementation is 1, BUT extending it to support more should be
		// relatively simple.
		Pipe->PacketSize = Ti->RowSize;
		Pipe->ItemMaxCount =
			ArenaCap(Pipe->Buffers[0]) / Pipe->PacketSize;
...
cleanup:
	if (Errno == -SDBE_JSON_ERR) {
		SdbLogError(
			"Encountered JSON error from sensor schema, cJSON error: %s",
			cJSON_GetErrorPtr());
	}
	if (SchemaConf != NULL) {
 ...
 */
static inline size_t Write2(char *To, const char *From)
{
	u16 Val;
...
static inline size_t Write4(char *To, const char *From)
{
	u32 Val;
	SdbMemcpy(&Val, From, sizeof(Val));
	Val = htobe32(Val);
...
static inline size_t Write8(char *To, const char *From)
{
	u64 Val;
	SdbMemcpy(&Val, From, sizeof(Val));
	Val = htobe64(Val);
...
static inline size_t WriteTimestamp(char *To, const char *From)
{
	time_t UTime;
	SdbMemcpy(&UTime, From, sizeof(UTime));
	pg_timestamp Val = UnixToPgTimestamp(UTime);
	Val = (pg_timestamp)htobe64((u64)Val);
	SdbMemcpy(To, &Val, sizeof(Val));
	return sizeof(Val);
}
 ...
 */
sdb_errno PgInsertData(PGconn *Conn, pg_table_info *Ti, const char *Data,
		       u64 ItemCount)
{
	PGresult *PgRes;
	sdb_errno Ret = 0;
	...
	if (PQresultStatus(PgRes) != PGRES_COMMAND_OK) {
		SdbLogError(
			"Failed to begin transaction for insertion into table %s. Pg error: %s",
	...
	if (PQresultStatus(PgRes) != PGRES_COPY_IN) {
		SdbLogError(
			"Failed to start COPY operation for table %s. Pg error: %s",
			Ti->TableName, PQerrorMessage(Conn));
		PQclear(PgRes);
	...
	SdbMemcpy(CopyHeader + 15, &CopyExtenstionLength,
		  sizeof(CopyExtenstionLength));

	if (PQputCopyData(Conn, CopyHeader, 19) != 1) {
		SdbLogError(
			"Failed to send COPY binary header for table %s. Pg error: %s",
			Ti->TableName, PQerrorMessage(Conn));
		Ret = -SDBE_PG_ERR;
		goto cleanup;
	}
	...
	size_t PgHeaderSize =
		sizeof(Ti->ColCount) +
		(ItemCount * sizeof(Ti->ColMetadata[0].TypeLength));
	size_t NtwrkConvBufSize = PgHeaderSize + (ItemCount * Ti->RowSize);

	SdhsArenaScratch NtwrkBufArena = ScratchGet(NULL, 0);
	char *NtwrkConvBuf =
		ArenaPushArrayZero(NtwrkBufArena.ua, char, NtwrkConvBufSize);
	if (NtwrkConvBuf == NULL) {
		SdbLogError(
			"Scratch arena has insufficient space for network conversion "
			"buffer (need "
			"%zd bytes). Re-evaluate arena buffer size",
			NtwrkConvBufSize);
		ScratchRelease(NtwrkBufArena);
		goto cleanup;
	}

	size_t ConvOffset = 0;
	for (u64 RowsProcessed = 0; RowsProcessed < ItemCount;
	     ++RowsProcessed) {
		const char *CurrRow =
			(const char *)Data + (RowsProcessed * Ti->RowSize);

		i16 NtwrkFieldCount =
			(i16)htons((u16)Ti->ColCountNoAutoIncrements);
		SdbMemcpy(NtwrkConvBuf + ConvOffset, &NtwrkFieldCount,
			  sizeof(NtwrkFieldCount));
		...
		for (int c = 0; c < Ti->ColCount; ++c) {
			...
			i32 NtwrkTypeLen = (i32)htobe32((u32)ColMd.TypeLength);
			SdbMemcpy(NtwrkConvBuf + ConvOffset, &NtwrkTypeLen,
				  sizeof(NtwrkTypeLen));
			ConvOffset += sizeof(NtwrkTypeLen);

			// WARN: The type length might not necessarily be the same as the type
			// written to the network conversion buffer, so we might need to add
			// handling of that
			const char *ColData = CurrRow + ColMd.Offset;
			switch (ColMd.TypeOid) {
			case PG_INT8:
			case PG_FLOAT8:
				ConvOffset += Write8(NtwrkConvBuf + ConvOffset,
						     ColData);
				break;
			case PG_INT4:
			case PG_FLOAT4:
				ConvOffset += Write4(NtwrkConvBuf + ConvOffset,
						     ColData);
				break;
			case PG_INT2:
				ConvOffset += Write2(NtwrkConvBuf + ConvOffset,
						     ColData);
				break;
			case PG_TIMESTAMP:
				...
				ConvOffset += WriteTimestamp(
					NtwrkConvBuf + ConvOffset, ColData);
				break;
			default:
				SdbLogWarning(
					"Encountered unhandled or invalid PG oid %d",
					ColMd.TypeOid);
				break;
			}
		}
	...
	if (PQputCopyData(Conn, NtwrkConvBuf, (int)ConvOffset) != 1) {
		SdbLogError(
			"Unable to copy converted data for table %s. Pg error: %s",
			Ti->TableName, PQerrorMessage(Conn));
...
cleanup:
	if (PQputCopyEnd(Conn, (Ret == 0) ? NULL : "Error during copy") != 1) {
		SdbLogError("Failed to end COPY for table %s. Pg error: %s",
			    Ti->TableName, PQerrorMessage(Conn));
		Ret = -SDBE_PG_ERR;
	}
	...
	if (PQresultStatus(PgRes) != PGRES_COMMAND_OK) {
		SdbLogError("COPY command failed for table %s. Pg error: %s",
			    Ti->TableName, PQerrorMessage(Conn));
		Ret = -SDBE_PG_ERR;
	}
	PQclear(PgRes);
	...
	if (PQresultStatus(PgRes) != PGRES_COMMAND_OK) {
		SdbLogError(
			"Failed to end transaction for table %s. Pg error: %s",
			Ti->TableName, PQerrorMessage(Conn));
		Ret = -SDBE_PG_ERR;
	}
	PQclear(PgRes);
	...
	if (Ret == 0) {
		SdbLogDebug("Committed copy transaction for table %s",
			    Ti->TableName);
	} else {
		SdbLogWarning("Rolled back copy transaction for table %s",
			      Ti->TableName);
	}
