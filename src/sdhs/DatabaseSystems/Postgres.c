                 ...
                 size_t *RowSize, SdhsArena *A)
{
    ...
    *ColCount = (i16)RowCount;
    ...
    pg_col_metadata *MetadataArray = ArenaPushArray(A, pg_col_metadata, (size_t)RowCount);
    if(!MetadataArray) {
...
PgPrepareCtx(SdhsArena *PgArena, sensor_data_pipe *Pipe)
{
    ...
    sdb_file_data *ConfFile = SdbLoadFileIntoMemory(POSTGRES_CONF_FS_PATH, Scratch.ua);
    if(ConfFile == NULL) {
    ...
    cJSON *SchemaConf = DbInitGetConfFromFile("./configs/sensor_schemas.json", Scratch.ua);
    ...
    cJSON_ArrayForEach(SensorSchema, SensorSchemaArray)
    {
        ...
        sdb_string CreationQuery = SdbStringMake(Scratch.ua, "CREATE TABLE IF NOT EXISTS ");
...
PgInsertData(PGconn *Conn, pg_table_info *Ti, const char *Data, u64 ItemCount)
{
    ...
    char            *NtwrkConvBuf  = ArenaPushArrayZero(NtwrkBufArena.ua, char, NtwrkConvBufSize);
