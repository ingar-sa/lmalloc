...
THREAD_ARENAS_REGISTER(Modbus, 2);
...
MbReceiveTcpFrame(int Sockfd, u8 *Frame, size_t BufferSize)
{
    ...
    while(TotalBytesRead < MODBUS_TCP_HEADER_LEN) {
        ssize_t BytesRead = recv(Sockfd, Frame + TotalBytesRead,
                                 (size_t)(MODBUS_TCP_HEADER_LEN - TotalBytesRead), 0);
        if(BytesRead <= 0) {
            return BytesRead;
        }
    ...
    u16 Length = (u16)(Frame[4] << 8) | Frame[5]; // Length from Modbus TCP header

    ssize_t TotalFrameSize = MODBUS_TCP_HEADER_LEN + Length;

    if(TotalFrameSize > (ssize_t)BufferSize) {
    ...
    while(TotalBytesRead < TotalFrameSize) {
        ssize_t BytesRead
            = recv(Sockfd, Frame + TotalBytesRead, (size_t)(TotalFrameSize - TotalBytesRead), 0);
        if(BytesRead <= 0) {
            return BytesRead;
        }
        TotalBytesRead += BytesRead;
 ...
 */
modbus_ctx *
MbPrepareCtx(SdhsArena *MbArena)
{
    modbus_ctx *MbCtx = ArenaPushStruct(MbArena, modbus_ctx);
    MbCtx->ConnCount  = 1;
    MbCtx->Conns      = ArenaPushArray(MbArena, mb_conn, MbCtx->ConnCount);
