...
{
    Timespec->tv_sec += (time_t)(SDB_TIME_TO_S((double)Delta));
    Timespec->tv_nsec += (time_t)(SDB_TIME_TO_NS(Delta) % (u64)1e9);

    if((double)Timespec->tv_nsec >= 1e9) {
        Timespec->tv_sec += 1;
        Timespec->tv_nsec %= (time_t)1e9;
    }
}
...
{
    Timespec->tv_sec  = (time_t)SDB_TIME_TO_S(Delta);
    Timespec->tv_nsec = (time_t)(SDB_TIME_TO_NS(Delta) % (u64)1e9);
}
...
{
    Timeval->tv_sec  = (time_t)SDB_TIME_TO_S(Delta);
...
{
    i64 TimeoutNs = (i64)((double)Timeout->tv_sec * 1e9 + (double)Timeout->tv_nsec);
    i64 NowNs     = (i64)((double)Now->tv_sec * 1e9 + (double)Now->tv_nsec);
